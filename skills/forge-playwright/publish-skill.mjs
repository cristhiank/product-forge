...
const defaultTarget = join(homedir(), '.copilot', 'skills', 'forge-playwright');
const target = resolveTarget(defaultTarget, process.argv.slice(2));

console.log('📦 Publishing forge-playwright skill...');
...
if (existsSync(join(scriptDir, 'references'))) {
  cpSync(join(scriptDir, 'references'), join(target, 'references'), { recursive: true });
}
if (existsSync(join(scriptDir, 'templates'))) {
  cpSync(join(scriptDir, 'templates'), join(target, 'templates'), { recursive: true });
}
