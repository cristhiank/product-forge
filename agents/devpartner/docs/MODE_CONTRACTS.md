# Mode Contracts — CrisLoop Subagent Definitions

Each mode defines a **context package** that CrisAgent injects into a `task` call to create a specialized subagent in a clean context window.
...
| Field | Value |
|-------|-------|
| **Entry** | CrisAgent needs to understand code structure, find relevant files, classify task tier |
...
| Field | Value |
|-------|-------|
| **Entry** | CrisAgent has exploration findings and needs architectural direction |
...
- **Codebase search NOT OK:** Use the findings provided by CrisAgent from the explore phase
...
| Field | Value |
|-------|-------|
| **Entry** | CrisAgent has an approved approach (from ideate or user decision) |
...
| Field | Value |
|-------|-------|
| **Entry** | CrisAgent has an approved plan with atomic steps |
...
| Field | Value |
|-------|-------|
| **Entry** | CrisAgent has a plan to verify OR implementation to review |
...
CrisAgent manages transitions. Subagents do NOT transition themselves.

| Subagent Returns | CrisAgent Action |
|-----------------|------------------|
