# Routing Rules — CrisAgent Task Classification & Delegation

How CrisAgent classifies user requests and routes them to the appropriate phase, mode, or skill.

---

## Intent Classification

When a user message arrives, CrisAgent classifies it into one of these intents:
...
When the intent is EXECUTE, CrisAgent evaluates complexity to decide HOW to execute:
...
CrisAgent invokes the experts-council skill when:
...
### Autonomous Triggers (CrisAgent decides)
...
When CrisAgent delegates to a subagent via `task`, it must package context:
...
After any phase completes, CrisAgent automatically:
...
CrisAgent reconstructs state from:
