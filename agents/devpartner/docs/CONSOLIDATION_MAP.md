# Consolidation Map — v17 Agents → CrisLoop

Detailed per-agent analysis of what content merges into CrisLoop, what gets eliminated as duplication, and what unique logic is preserved in mode files.
...
After restructuring into CrisLoop format (~1,550 lines), the effective reduction is **~64%**.

---

## Per-Agent Breakdown

### Orchestrator (343 lines → CrisAgent core + SKILL.md routing)

**Preserved (~150 lines → CrisAgent.agent.md + SKILL.md):**
- Tier routing table (Direct/Delegate modes) → becomes task routing in SKILL.md
- Delegate Mode state machine (Phases 0-4c) → becomes CrisLoop phase machine
...
- `task` delegation parameter docs (30 lines) — CrisAgent packages prompts directly
- subagent prompt examples for each role (70 lines) — replaced by mode file injection
- `scout_requests` handling (15 lines) — CrisAgent delegates directly
...
- `scout_requests` pattern (15 lines) — CrisAgent handles context
...
- Context packaging for Creative input (30 lines) — CrisAgent packages
...
**Only change:** Update worker spawn to use `--agent CrisAgent` instead of `--agent Orchestrator`.
...
- CrisAgent packages mode contract + context directly into `task` prompts
- Subagents return plain text/markdown, not XML envelopes
- No inter-agent request protocol — CrisAgent mediates everything
...
In CrisLoop, mode files are self-contained context packages — they include their own rules without referencing external constitutions.

### Permission Matrices (~100 lines across all agents)

Each v17 agent has explicit tool permission tables. In CrisLoop, the `task` tool gives subagents access to all tools by default. Mode contracts specify restrictions (e.g., "In explore mode, do NOT edit files") as behavioral rules rather than permission matrices.
