...
function parseUsageDelta(data: Record<string, unknown>): UsageDelta {
  ...
  const toolTelemetry = asRecord(data.toolTelemetry);
  const toolTelemetryMetrics = asRecord(toolTelemetry?.metrics);
  const sources: Array<Record<string, unknown> | null> = [
    ...
    asRecord(result?.tokensUsed),
    asRecord(result?.tokenUsage),
    asRecord(result?.usageMetrics),
    asRecord(toolTelemetry?.usage),
    asRecord(toolTelemetry?.tokenUsage),
...
function hasUsage(usage: UsageDelta): boolean {
  return usage.inputTokens + usage.outputTokens + usage.cachedInputTokens + usage.cachedOutputTokens > 0;
}
...
function ensureUsageBuckets(
  telemetry: OpsTelemetryMetadata,
  model: string,
  provider: string,
): void {
  if (!model || model === 'unknown') return;
  ensureModelUsage(telemetry.models, model, provider || 'unknown');
  ensureProviderUsage(telemetry.providers, provider || 'unknown');
}

function bumpRequestOnly(
  telemetry: OpsTelemetryMetadata,
  model: string,
  provider: string,
  timestamp: string,
  ...
  if (!model || model === 'unknown') return;
  const modelSummary = ensureModelUsage(telemetry.models, model, provider || 'unknown');
  modelSummary.requests += 1;
  modelSummary.lastUsedAt = timestamp;

  const providerSummary = ensureProviderUsage(telemetry.providers, provider || 'unknown');
...
function normalizeModelCandidate(candidate: unknown): string | null {
  if (typeof candidate === 'string' && candidate.trim().length > 0) return candidate.trim();
  const record = asRecord(candidate);
  if (!record) return null;
  const nestedCandidates = [record.model, record.name, record.id, record.modelId, record.model_id];
...
function resolveModelFromEvent(data: Record<string, unknown>): string | null {
  const result = asRecord(data.result);
  const toolTelemetryProperties = asRecord(asRecord(data.toolTelemetry)?.properties);
  ...
  const candidates = [
    ...
    result?.model,
    asRecord(result?.context)?.model,
    asRecord(result?.session)?.model,
    toolTelemetryProperties?.model,
    metadata?.model,
  ];
...
function updateOpsTelemetryFromEvents(
  ...
  for (const event of events) {
    ...
    ensureUsageBuckets(next, model, provider);
    if ((event.type === 'assistant.message' || event.type === 'turn.assistant_message') && !hasUsage(usage)) {
      bumpRequestOnly(next, model, provider, event.timestamp);
    }
...
export class Hub {
  ...
  workerSync(id: string): WorkerSyncResult {
    ...
    const shouldBackfillOpsTelemetry = !hasStoredOpsTelemetry
      || (
        opsTelemetry.activeModel !== null
        && opsTelemetry.usageTotals.totalTokens === 0
        && Object.keys(opsTelemetry.models).length === 0
        && Object.keys(opsTelemetry.providers).length === 0
      );
    ...
    if (shouldBackfillOpsTelemetry && worker.eventsOffset > 0) {
    ...
    if (events.length === 0) {
      ...
      if (shouldBackfillOpsTelemetry || telemetryBackfilled) {
